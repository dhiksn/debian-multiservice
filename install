...
# Fungsi untuk mendapatkan network dan subnet mask dari IP
get_network_from_ip() {
    local ip="$1"
    local interface_info=$(ip -o -4 addr show | grep "$ip" | head -1)
    local cidr="${interface_info##*/}"
    cidr="${cidr%% *}"
    ...
    # Konversi CIDR ke netmask dan network
    local IFS=.
    read -r i1 i2 i3 i4 <<< "$ip"
    local mask=$(( 0xFFFFFFFF << (32 - cidr) ))
    local network=$(( (i1<<24 | i2<<16 | i3<<8 | i4) & mask ))
    
    printf "%d.%d.%d.%d" $(( (network>>24) & 255 )) $(( (network>>16) & 255 )) $(( (network>>8) & 255 )) $(( network & 255 ))
}
...
install_dns() {
    ...
    log_info "Menginstall bind9 dan utils..."
    apt install -y bind9 bind9utils bind9-doc dnsutils

    log_info "Memilih IP Address untuk DNS Server..."
    DNS_IP=$(select_ip "Pilih IP Address untuk DNS Server")
    
    if [[ -z "$DNS_IP" ]]; then
        log_error "Gagal memilih IP Address. Menggunakan default."
    ...
    # Input domain name
    echo -e "${CYAN}Masukkan nama domain yang diinginkan (contoh: techcorp.local):${NC}"
    printf "Domain name: "
    read -r DOMAIN_NAME < /dev/tty
    
    if [[ -z "$DOMAIN_NAME" ]]; then
        DOMAIN_NAME="techcorp.local"
        log_warn "Domain name tidak diisi, menggunakan default: ${DOMAIN_NAME}"
    fi

    # Tanya apakah ingin integrasi dengan IP Apache
    echo -e "${CYAN}Apakah ingin mengintegrasikan DNS dengan IP Apache? (y/n)${NC}"
    printf "Integrasi dengan Apache [y/n] (default y): "
    read -r INTEGRATE_APACHE < /dev/tty
    
    if [[ -z "$INTEGRATE_APACHE" ]] || [[ "$INTEGRATE_APACHE" == "y" ]] || [[ "$INTEGRATE_APACHE" == "Y" ]]; then
        INTEGRATE_APACHE="y"
        if [[ -z "$APACHE_ACCESS_IP" ]]; then
            APACHE_ACCESS_IP=$(select_ip "Pilih IP Address untuk integrasi dengan Apache")
        fi
        WEB_IP="$APACHE_ACCESS_IP"
        log_info "DNS akan mengarahkan ${DOMAIN_NAME} dan www.${DOMAIN_NAME} ke IP Apache: ${WEB_IP}"
    else
        echo -e "${CYAN}Masukkan IP Address untuk web server (contoh: ${DNS_IP}):${NC}"
        printf "Web Server IP: "
        read -r WEB_IP < /dev/tty
        if [[ -z "$WEB_IP" ]]; then
            WEB_IP="$DNS_IP"
        fi
    fi

    # Tanya DNS forwarders
    echo -e "${CYAN}Masukkan DNS Forwarders (pisahkan dengan spasi, default: 8.8.8.8 8.8.4.4):${NC}"
    printf "DNS Forwarders: "
    read -r DNS_FORWARDERS < /dev/tty
    if [[ -z "$DNS_FORWARDERS" ]]; then
        DNS_FORWARDERS="8.8.8.8; 8.8.4.4;"
    else
        DNS_FORWARDERS=$(echo "$DNS_FORWARDERS" | sed 's/ /; /g')";"
    fi

    log_info "Membuat backup konfigurasi BIND9..."
...
BINDOPT
    ...
    log_info "Membuat file zone untuk domain ${DOMAIN_NAME}..."
...
FORWARDZONE
    ...
    REVERSE_IP=$(echo "$DNS_IP" | awk -F. '{print $4}')
    cat > "/etc/bind/db.${REVERSE_NET}" << REVERSEZONE
;
; BIND reverse data file for ${REVERSE_NET}.in-addr.arpa
;
\$TTL    604800
@       IN      SOA     ns1.${DOMAIN_NAME}. admin.${DOMAIN_NAME}. (
                      $(date +%Y%m%d%H)     ; Serial
...
@       IN      NS      ns1.${DOMAIN_NAME}.
@       IN      NS      ns2.${DOMAIN_NAME}.
${REVERSE_IP}    IN      PTR     ns1.${DOMAIN_NAME}.
${REVERSE_IP}    IN      PTR     www.${DOMAIN_NAME}.
REVERSEZONE

    log_info "Memeriksa konfigurasi BIND9..."
    named-checkconf
    named-checkzone "${DOMAIN_NAME}" "/etc/bind/db.${DOMAIN_NAME}"
    named-checkzone "${REVERSE_NET}.in-addr.arpa" "/etc/bind/db.${REVERSE_NET}"
    ...
    log_info "Mengupdate resolv.conf untuk menggunakan DNS server lokal..."
...
install_dhcp() {
    ...
    if [[ -z "$DHCP_INTERFACE" ]]; then
        log_error "Gagal memilih interface. Menggunakan default: eth0"
        DHCP_INTERFACE="eth0"
    fi
    
    log_info "DHCP Server akan berjalan pada interface: ${DHCP_INTERFACE}"

    # Mendapatkan network information dari interface yang dipilih
    INTERFACE_IP=$(ip -o -4 addr show | grep "$DHCP_INTERFACE" | awk '{print $4}' | cut -d/ -f1)
    NETWORK_ADDR=$(get_network_from_ip "$INTERFACE_IP")
    
    # Tentukan default range berdasarkan network
    IFS=. read -r n1 n2 n3 n4 <<< "$NETWORK_ADDR"
    DEFAULT_START="${n1}.${n2}.${n3}.100"
    DEFAULT_END="${n1}.${n2}.${n3}.200"
    
    log_info "Network terdeteksi: ${NETWORK_ADDR}/24"
    
    # Input custom DHCP range
    echo -e "${CYAN}Masukkan range IP untuk DHCP (mulai - akhir)${NC}"
    printf "Range mulai (default ${DEFAULT_START}): "
    read -r DHCP_START < /dev/tty
    printf "Range akhir (default ${DEFAULT_END}): "
    read -r DHCP_END < /dev/tty
    ...
    # Input gateway (router)
    DEFAULT_GATEWAY="${n1}.${n2}.${n3}.1"
    echo -e "${CYAN}Masukkan alamat Gateway/Router (default ${DEFAULT_GATEWAY}):${NC}"
    printf "Gateway: "
    read -r DHCP_GATEWAY < /dev/tty
    if [[ -z "$DHCP_GATEWAY" ]]; then
        DHCP_GATEWAY="$DEFAULT_GATEWAY"
    fi
    
    # Input DNS servers
    echo -e "${CYAN}Masukkan DNS Server (pisahkan dengan koma, default: ${INTERFACE_IP},8.8.8.8):${NC}"
    printf "DNS Servers: "
    read -r DHCP_DNS < /dev/tty
    if [[ -z "$DHCP_DNS" ]]; then
        DHCP_DNS="${INTERFACE_IP},8.8.8.8"
    fi
    
    # Input domain name
    echo -e "${CYAN}Masukkan Domain Name (default: techcorp.local):${NC}"
    printf "Domain Name: "
    read -r DHCP_DOMAIN < /dev/tty
    if [[ -z "$DHCP_DOMAIN" ]]; then
        DHCP_DOMAIN="techcorp.local"
    fi
    
    # Input lease time
    echo -e "${CYAN}Masukkan waktu lease (detik, default: 86400 = 1 hari):${NC}"
    printf "Lease time (detik): "
    read -r DHCP_LEASE < /dev/tty
    ...
    log_info "Mengkonfigurasi dhcpd.conf..."
    cat > /etc/dhcp/dhcpd.conf << DHCPCONF
# DHCP Configuration for TechCorp Network
# Generated on $(date)

option domain-name "${DHCP_DOMAIN}";
option domain-name-servers ${DHCP_DNS};
default-lease-time ${DHCP_LEASE};
max-lease-time $((DHCP_LEASE * 2));
...
subnet ${NETWORK_ADDR} netmask 255.255.255.0 {
    ...
    option broadcast-address ${n1}.${n2}.${n3}.255;
    option domain-name "${DHCP_DOMAIN}";
    option domain-name-servers ${DHCP_DNS};
    ...
    # Optional: static reservations can be added here
    # host client1 {
    #     hardware ethernet XX:XX:XX:XX:XX:XX;
    #     fixed-address ${n1}.${n2}.${n3}.50;
    # }
}
...
DHCPCONF
    ...
    log_info "Mengkonfigurasi interface pada /etc/default/isc-dhcp-server..."
...
DHCPDEFAULT
    ...
    if command -v ufw &> /dev/null; then
        ufw allow 67/udp
        log_info "UFW: Port 67 (UDP) dibuka untuk DHCP."
    fi
    ...
    log_info "   IP Range: ${DHCP_START} - ${DHCP_END}"
    log_info "   Gateway: ${DHCP_GATEWAY}"
    log_info "   DNS: ${DHCP_DNS}"
    log_info "   Domain: ${DHCP_DOMAIN}"
    log_info "   Lease Time: ${DHCP_LEASE} detik"
    
    # Tampilkan status lease file
    log_info "Lease database: /var/lib/dhcp/dhcpd.leases"
}
...
install_all() {
    ...
    echo -e "${GREEN}${BOLD}  │${NC}  🌐 DNS        : DNS Server berjalan di port 53                               ${GREEN}${BOLD}│${NC}"
    echo -e "${GREEN}${BOLD}  │${NC}  📡 DHCP       : DHCP Server berjalan (cek lease di /var/lib/dhcp/)           ${GREEN}${BOLD}│${NC}"
